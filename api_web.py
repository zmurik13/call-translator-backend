...
async def process_voice_translation(
	...
	# 2. LLM
	if is_silence:
		translated_text = "Atsiprašau, neišgirdau." if source_lang == "ru" else "Извините, я не расслышал."
	else:
		translated_text = await ai_core.translate_and_fix(raw_text, source_lang)

	# 3. Discord
	msg = f"**Source ({source_lang}):** {raw_text}\n**Translated:** {translated_text}\n**Device:** {device_info}"
	background_tasks.add_task(send_discord_alert, "🗣️ Web Translation Log", msg, 3447003)

	# 4. TTS
	target_lang = "lt" if source_lang == "ru" else "ru"
